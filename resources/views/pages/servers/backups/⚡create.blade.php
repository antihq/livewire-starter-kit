...
new class extends Component
{
    ...
    public bool $notificationOnFailure = false;

    public bool $notificationOnSuccess = false;
    ...
    public function create(): void
    {
        ...
        $backup = $this->team->backups()->create([
            'name' => $this->name,
            ...
            'notification_on_failure' => $this->notificationOnFailure,
            'notification_on_success' => $this->notificationOnSuccess,
            'notification_email' => $this->notificationEmail,
            'creator_id' => $this->user->id,
        ]);

        if (! empty($this->selectedDatabases)) {
            $backup->databases()->attach($this->selectedDatabases);
        }

        $this->redirectRoute('servers.backups.index', $this->server->id, navigate: true);
    }
};
?>
...
<div class="mx-auto max-w-lg">
    <form wire:submit="create" class="space-y-8">
        <flux:heading size="lg">Add Backup to {{ $server->name }}</flux:heading>
        ...
        @else
            <flux:callout>
                No backup disks configured. <flux:button variant="ghost" :href="route('backup-disks.create')" wire:navigate>Create one now</flux:button>.
            </flux:callout>
        @endif
        ...
        @else
            <flux:callout>
                No databases configured on this server. <flux:button variant="ghost" :href="route('servers.databases.create', $server->id)" wire:navigate>Create one now</flux:button>.
            </flux:callout>
        @endif
        ...
        <div class="flex gap-3">
            <flux:spacer />
            <flux:button variant="ghost" :href="route('servers.backups.index', $server->id)" wire:navigate>Cancel</flux:button>
            <flux:button variant="primary" type="submit">Create Backup</flux:button>
        </div>
    </form>
</div>
